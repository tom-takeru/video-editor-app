...
class Videdi:
    ...
    def silence_sections(self, video):
        try:
            ...
            output = subprocess.run(["/Applications/videdi.app/Contents/MacOS/ffmpeg", "-i", video, "-af",
    ...
    def leave_sections(self, sections, video):
        try:
            ...
            output = subprocess.run(
                ["/Applications/videdi.app/Contents/MacOS/ffprobe", video, "-hide_banner", "-show_format"],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            s = str(output)
            lines = s.split("\\n")
            for line in lines:
    ...
    def cut_video(self, video_dir, sections, video):
        ...
        for i in range(len(sections)):
            ...
            subprocess.run(
                ["/Applications/videdi.app/Contents/MacOS/ffmpeg", "-i", video, "-ss", str(sections[i][0]), "-t",
    ...
    def speech_recognize(self, video_dir, video_list):
        ...
        for i, video in enumerate(video_list):
            try:
                ...
                command = ["/Applications/videdi.app/Contents/MacOS/ffmpeg", "-i", video, audio]
    ...
    def print_sub(self, video_dir, video):
        try:
            os.chdir(video_dir)
            command = ['/Applications/videdi.app/Contents/MacOS/ffmpeg', '-i', video,
